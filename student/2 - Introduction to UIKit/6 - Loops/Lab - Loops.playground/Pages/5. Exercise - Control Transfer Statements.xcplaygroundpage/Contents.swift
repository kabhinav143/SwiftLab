...
for (multiple, abc) in alphabet.enumerated(){
    if multiple.isMultiple(of: 2){
        print("\(multiple): \(abc)")
    }
}
...
var states:[String:String]=["Himachal Pradesh":"Shimla","Haryana":"Chandigarh","Jammu and Kashmir":"Srinagar"]
for (naam,capital) in states{
    if(naam=="Haryana"){
        print("I found my Home!")
        break;
    }
    else{
        print("\(naam)'s Capital is: \(capital)")
    }
}
